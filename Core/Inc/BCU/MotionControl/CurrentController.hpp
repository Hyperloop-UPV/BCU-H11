...
namespace BCU{
    ...
    static inline Types::DutyCycles execute(Types::TelemetryData& data){
        ...
        #endif
        electrical_angle = fmod(M_PI * (position - (2.0f * floor(position / 2.0f))) + angle_offset,2*M_PI);
    ...
    static inline void set_angle_offset(float angle){
        angle_offset = angle;
    }
    static inline void set_d_ref(float d_ref){
        reference.d = d_ref;
    }
    static inline void set_q_ref(float q_ref){
        reference.q = q_ref;
    }
    private:
    ...
    static inline void inverse_clarke_transform(float alpha,float beta,float& u,float& v,float& w){
        u = alpha;
    ...
    static inline void update_d(float current_error){
        ...
        if(output.d > ControlConf::D_MAX_OUTPUT) output.d = ControlConf::D_MAX_OUTPUT;
        if(output.d < ControlConf::D_MIN_OUTPUT) output.d = ControlConf::D_MIN_OUTPUT;
    }
    static inline void update_q(float current_error){
        ...
        if(output.q > ControlConf::D_MAX_OUTPUT) output.q = ControlConf::D_MAX_OUTPUT;
        if(output.q < ControlConf::D_MIN_OUTPUT) output.q = ControlConf::D_MIN_OUTPUT;
    }
    inline static PI<IntegratorType::Trapezoidal,float> d_current_control{ControlConf::D_KP_Current,ControlConf::D_KI_Current,ControlConf::CurrentControlPeriod};
    inline static PI<IntegratorType::Trapezoidal,float> q_current_control{ControlConf::Q_KP_Current,ControlConf::Q_KI_Current,ControlConf::CurrentControlPeriod};
    struct Output{
        float d;
        float q;
    ...
    struct Data{
        ...
        float& angle_offset;
        Data(Reference& ref,Error& err,Output& out, Measured& measured,Types::DutyCycles& target,float& electrical_angle,float& angle):
            reference(ref),error(err),output(out),measured(measured),target(target),electrical_angle(electrical_angle),angle_offset(angle){}
    };   
