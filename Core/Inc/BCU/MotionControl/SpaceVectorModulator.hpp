...
namespace BCU{
    class SpaceVectorModulator{
        ...
        inline static Data subscribe(){
            return Data(modulation_frequency,modulation_index);
        }
        inline static void set_modulation_index(float voltage_ref,float voltage_max){
            ...
            if (voltage_ref < 0) voltage_ref = 0.0f;
            modulation_index = (voltage_ref * 2.0f) / voltage_max;
            if (modulation_index >= MAX_IMOD) modulation_index = MAX_IMOD;
        }
        inline static Types::DutyCycles execute(){
            ...
            angle = RotationComputer::radian_f32_to_q31(TWO_PI * modulation_frequency * control_time + phase_shift);
            RotationComputer::sin(&angle,&sin_q31,1);
        ...
        inline static void set_modulation_freq(uint32_t freq){
        ...
        inline static constexpr float phase_shift = TWO_PI / 3.0;
        inline static constexpr float period_time = (ControlConf::SpaceVectorPeriod / 1'000'000);
